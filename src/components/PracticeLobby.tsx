  ...
  return (
    ...
    >
        ...
        <div className="mb-5 flex items-end justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-stone-400">
              {isChat
                ? "Start a text chat · private 1-on-1"
                : "Start an audio call · private 1-on-1"}
            </p>
          </div>
      ...
      {!isChat ? (
        ...
        <div className="mt-6 animate-[fade-up_0.7s_ease-out_0.14s_both]">
          {error ? (
            <p className="text-sm text-amber-700 dark:text-amber-300">{error}</p>
          ) : null}
          {banner && phase !== "outgoing" ? (
            <p className="text-sm text-stone-600 dark:text-stone-300">{banner}</p>
          ) : null}
        </div>
      )}
      ...
      {isChat ? (
        <div className="animate-[fade-up_0.75s_ease-out_0.18s_both]">
          <ChatInbox
            ...
            myId={profile.id}
            onlineUsers={users}
            onMessageAgain={onMessageAgain ?? (() => undefined)}
            preview={preview}
          />
        </div>
      ) : (
        <div className="animate-[fade-up_0.75s_ease-out_0.18s_both]">
          <CallHistory entries={callHistory} preview={preview} />
        </div>
      )}

      {/* keep statusLabel referenced for future header restore */}
